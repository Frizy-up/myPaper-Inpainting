...
The generator is a simple encoder-decoder pipeline. This architecture try to reconstruct image after passing it to a low-dimensional bottleneck layer. By doing this, the networks learned the image content and semantically \cite{Hinton2006:s:Reducing,Bengio:Co:Learning,Vincent2008::Extracting}. Pathak et al. \cite{Pathak2016::Context} first uses this architecture to do image inpainting, proposed the Context Encoders method. They use L2 distance to capture the overall structure of the missing region in relation to the context. Isola et al. \cite{Isola2017::Image} explore this option, using L1 distance to reduce the blurring in the reconstructed images. Our method also adopts L1 distance to reconstruct the original left image, the difference is that we joint the abnormal left image($\tilde{x}$) and corresponding right image($y$) to achieve the goal:
